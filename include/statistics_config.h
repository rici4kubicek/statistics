...
*Define `STATISTICS_PORT_USER` at compile time and provide compatible * implementations of the following macros prior to including this header if * you need to map them to a platform - specific allocator : *- `statPortMalloc(size)` * - `statPortFree(ptr)` * - `statPortMemcpy(dest, src, count)` * - `statPortCalloc(num, size)` * - `statPortMemset(ptr, val, count)` ** @{* /
#endif
